...
//
// This lesson introduces a proper stable raster IRQ pattern.
//
// Earlier raster IRQ lessons gave us a chosen raster line,
// but not a predictable cycle inside that line.
//
// This lesson uses the double IRQ method:
//
//   first IRQ  - setup interrupt
//   second IRQ - stabilised interrupt
//
// The first IRQ sets up a second IRQ on the next raster line,
// then re-enables interrupts and waits using NOP instructions.
//
// The second IRQ interrupts the first handler while it is executing
// predictable 2-cycle NOPs. That reduces the timing uncertainty.
//
// The second IRQ then performs a final one-cycle correction
// before running the visible timing marker.
//
// This version only changes:
//
//   $d020 - border colour
//
// It does not change $d021 yet.
// We are isolating stable timing before drawing full-width raster bars.
...
.const setup_line = 92        // First IRQ line. Chosen away from badlines
...
//
// Its job is not to draw the visible marker.
...
irq_setup:
    ...
    tsx                       // Save the current stack pointer in X
...
stable_point:
    ...
    sta $d020                 // Start visible timing marker

    ldx #$10                  // Hold marker long enough to judge stability

marker_hold:
    dex                       // Count down
    bne marker_hold           // Repeat until X reaches zero

    lda #$00                  // Load colour 0, black
    sta $d020                 // End visible timing marker
